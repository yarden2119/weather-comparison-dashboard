...
def fetch_dynamic_city_history(city_name, latitude, longitude):
    ...
    city_df = validate_and_clean_data(city_df)

    city_df["average_temperature_c"] = (
        (
            city_df["max_temperature_c"]
            + city_df["min_temperature_c"]
        )
        / 2
    ).round(1)

    city_df["temperature_range_c"] = (
        city_df["max_temperature_c"]
        - city_df["min_temperature_c"]
    ).round(1)
    ...
    city_df["month_name"] = (
        city_df["date"].dt.month_name()
    )

    city_df["is_rainy_day"] = (
        city_df["precipitation_mm"] > 0
    )
